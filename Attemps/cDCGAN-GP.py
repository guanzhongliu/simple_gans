...
def compute_gradient_penalty(model, real, fake, real_label, fake_label):
    ...
    gradients_label = autograd.grad(
        outputs=D_interpolates,
        inputs=interpolates_label,
        grad_outputs=fake_out,
        create_graph=True,
        retain_graph=True,
        only_inputs=True
    )[0]
    gradients_label = gradients_label.view(gradients_label.size(0), -1)
    gradients_pic = gradients_pic.view(gradients_pic.size(0), -1)
    gradient_penalty = ((gradients_pic.norm(2, dim=1) - 1) ** 2).mean() + ((gradients_label.norm(2, dim=1) - 1) ** 2).mean()
...
for epoch in range(train_epoch):
    ...
    for idx, (feature, label) in enumerate(train_loader):
        ...
        D_penalty_loss = penalty_factor * compute_gradient_penalty(discriminator, feature, G_result, real_fill, gen_fill)
        discriminator.zero_grad()
