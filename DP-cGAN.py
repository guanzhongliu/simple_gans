...
def parseArg():
    ...
    conf_file = os.path.abspath("conf/mnist_cGAN.json")
...
class Generator(nn.Module):
    def __init__(self, z_dim, cond_num):
        super(Generator, self).__init__()
        self.fc1 = nn.Linear(z_dim + cond_num, 128 * pic_dim)
        self.fc2 = nn.Linear(128 * pic_dim, img_size * img_size * pic_dim)
...
class Discriminator(nn.Module):
    def __init__(self, img_size, cond_num):
        super(Discriminator, self).__init__()
        self.fc1 = nn.Linear(img_size * img_size * pic_dim + cond_num, 128 * pic_dim)
        self.fc2 = nn.Linear(128 * pic_dim, 1)
        for param in self.parameters():
            nn.init.normal_(param, mean=0, std=0.02)
...
def save_result(num_epoch, path='result.png'):
    with torch.no_grad():
        test_images = generator(test_z, test_label).view(-1, pic_dim, img_size, img_size)
...
discriminator = Discriminator(img_size, cond_num)
...
elif lr_decay_type == "SGDR":
    D_schedular = lr_scheduler.CosineAnnealingLR(D_optimizer, T_max=(train_epoch // 3) + 1)
    G_schedular = None
elif lr_decay_type == "None":
    D_schedular = lr_scheduler.StepLR(D_optimizer, step_size=1, gamma=0.95)
    print(D_schedular)
    G_schedular = None
else:
...
for epoch in range(start_epoch, train_epoch):
    ...
    for idx, (feature, label) in enumerate(train_loader):
        ...
        '''训练判别器'''
        D_total_loss = 0
        feature = feature.reshape(-1, img_size * img_size * pic_dim)
        for step in range(n_disc):
            ...
            # 生成所用标签
            # print('DIS_FAKE ',idx)
            gen_z = torch.randn((batch_size, z_dim)).view(-1, z_dim)
            label = (torch.rand(batch_size, 1) * 10).type(torch.LongTensor).squeeze()
            gen_fill = one_hot[label]
            gen_z, gen_fill = Variable(gen_z.to(device)), Variable(
                gen_fill.to(device))
            # 用生成图像训练
            G_result = generator(gen_z, gen_fill)
    ...
    # 测试生成的图片训练效果
    val_z = torch.randn((generate_num, z_dim)).view(-1, z_dim)
